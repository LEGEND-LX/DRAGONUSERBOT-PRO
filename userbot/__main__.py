...
if len(sys.argv) not in (1, 3, 4):
    ...
    try:
        if Config.BOT_USERNAME is not None:
            ...
            LOGS.info("♥️ Starting SATYABOT-PRO ♥️")
            bot.loop.run_until_complete(add_bot(Config.BOT_USERNAME))
            LOGS.info("💥💫 SATYABOT-PRO Startup Completed 💥💫")
        else:
            bot.start()
    except Exception as e:
...
async def help(event):
    ...
    if event.query.user_id is not bot.uid:
        ...
        await tgbot.send_message(
            ...
            buttons=[
                [
                    Button.url(" Support ", "https://t.me/DragonBot_Pro"),
                    Button.url(" Updates ", "https://t.me/DragonBot_Officel"),
                ],
                [
...
async def rel(event):
    if event.query.user_id == bot.uid:
        await event.answer(
            "Reloading Dragonẞø†... Wait for few seconds...", cache_time=0, alert=True
        )
